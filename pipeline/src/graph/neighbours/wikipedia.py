...
def get_see_also_labels(wikipedia_label:str):
    sections_response = fetch_json(base_url, params={
        "action":"parse",
        "prop":"sections",
    ...
    try:
        see_also_section = [section["index"] for section in sections_response["parse"]["sections"]][0]
        links_response = fetch_json(base_url, params={
            "action":"parse",
            "prop":"links",
            "page":wikipedia_label,
        ...
        see_also_labels = [link["*"] for link in links_response["parse"]["links"]]
...
def get_wikipedia_neighbours(target_concept: Concept, wikipedia_label: str):
    ...
    for neighbour_wikipedia_label in category_labels + see_also_labels:
        log.debug(
            "Found related wikipedia page", wikipedia_label=neighbour_wikipedia_label
        )
        ...
        if neighbour_source_concept:
            ...
            try:
                neighbour_concept_wikipedia_data = get_wikipedia_data(neighbour_wikipedia_label)
                label = get_wikipedia_preferred_label(neighbour_concept_wikipedia_data)
                log.info(
